...
CLDepthEstimator::CLDepthEstimator(
...
void CLDepthEstimator::createDepthMap(
...
void CLDepthEstimator::printInfo(){
	//Error handle.
...
void CLDepthEstimator::profileEvent(
...
cl_platform_id CLDepthEstimator::findPlatform(){
	//Error handle.
	cl_int err = CL_SUCCESS;
...
cl_device_id CLDepthEstimator::findDevice(
...
//Create a CL context.
cl_context CLDepthEstimator::createContext(
	cl_device_id* device
){
	//Error handle.
	cl_int err = CL_SUCCESS;
...
cl_command_queue CLDepthEstimator::createQueue(
	cl_context context,
	cl_device_id device
){
	//Error handle.
...
cl_kernel CLDepthEstimator::createKernel(
	const char* name,
	const char* source
){
	//Error handle.
...
void CLDepthEstimator::prepareKernels(){
	{
			...
			__kernel void downsample(
				...
				if((m<w)&&(n<h)){
					...
					for(int i=N;i<N+factor;i++){
						for(int j=M;j<M+factor;j++){
							val += img[j+i*width];
						}
					}

					out[m+n*w] = val / (factor * factor);
				}
			}
		)";
...

	{
		...
		const char* source = R"(
			__kernel void disparity(
				__global const unsigned char* img_0,
				__global const unsigned char* img_1,
				__global const unsigned char* mean_0,
				__global const unsigned char* mean_1,
				const unsigned int width,
				const unsigned int height,
				const unsigned int radius,
				const unsigned int maxDisparity,
				const int direction,
				__global unsigned char* out
			){
				int m = get_global_id(0);
...

	{
		...
		const char* source = R"(
			__kernel void crosscheck(
				...
				const unsigned int width,
				const unsigned int height,
				const unsigned int maxDifference
			){
				int m = get_global_id(0);
...
cl_mem CLDepthEstimator::createBuffer(
	cl_mem_flags flags,
	uint32_t size,
	void* copy
){
...
void CLDepthEstimator::loadImage(
...
void CLDepthEstimator::writeImage(
	cl_command_queue queue,
	const char* filename,
	uint32_t width,
	uint32_t height,
...
void CLDepthEstimator::makeImgGrey(
	cl_command_queue queue,
	cl_mem* img,
	...
	const size_t global[1] = {width*height};
	err = clEnqueueNDRangeKernel(queue, k_greyscale, 1, 0, global, NULL, 0, NULL, event);
	if(err != CL_SUCCESS){
...
void CLDepthEstimator::makeImgRGBA(
	cl_command_queue queue,
	cl_mem* img,
	const uint32_t width,
	const uint32_t height,
	...
	const size_t global[1] = {width*height};
	err = clEnqueueNDRangeKernel(queue, k_rgba, 1, 0, global, NULL, 0, NULL, event);
	if(err != CL_SUCCESS){
...
void CLDepthEstimator::downsampleImg(
	cl_command_queue queue,
	cl_mem* img,
	const uint32_t width,
	const uint32_t height,
	...
	const size_t global[2] = {width/factor, height/factor};
	err = clEnqueueNDRangeKernel(queue, k_downsample, 2, 0, global, NULL, 0, NULL, event);
	if(err != CL_SUCCESS){
...
void CLDepthEstimator::filterImg(
	cl_command_queue queue,
	cl_mem* img,
	const uint32_t width,
	const uint32_t height,
	...
	err = clEnqueueNDRangeKernel(queue, k_filter, 2, 0, global, NULL, 0, NULL, event);
	if(err != CL_SUCCESS){
...
void CLDepthEstimator::calcDisparity(
	cl_command_queue queue,
	cl_mem* img_0,
	cl_mem* img_1,
	cl_mem* mean_0,
	...
	const size_t global[2] = {width, height};
	err = clEnqueueNDRangeKernel(queue, k_disparity, 2, 0, global, NULL, 0, NULL, event);
	if(err != CL_SUCCESS){
...
void CLDepthEstimator::crossCheck(
	cl_command_queue queue,
	cl_mem* left,
	cl_mem* right,
	const uint32_t width,
	...
	const size_t global[1] = {width * height};
	err = clEnqueueNDRangeKernel(queue, k_cross, 1, 0, global, NULL, 0, NULL, event);
	if(err != CL_SUCCESS){
...
void CLDepthEstimator::occlusionFill(
	cl_command_queue queue,
	cl_mem* img,
	const uint32_t width,
	const uint32_t height,
	...
	const size_t global[2] = {width, height};
	err = clEnqueueNDRangeKernel(queue, k_occlusion, 2, 0, global, NULL, 0, NULL, event);
	if(err != CL_SUCCESS){
